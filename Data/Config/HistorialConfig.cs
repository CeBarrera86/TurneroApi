...
public class HistorialConfig : IEntityTypeConfiguration<Historial>
{
  public void Configure(EntityTypeBuilder<Historial> builder)
  {
    ...
    builder.HasOne(h => h.TicketNavigation).WithMany(t => t.Historiales).HasForeignKey(h => h.TicketId).OnDelete(DeleteBehavior.Cascade);
    builder.HasOne(h => h.EstadoNavigation).WithMany(e => e.Historiales).HasForeignKey(h => h.EstadoId).OnDelete(DeleteBehavior.Restrict);
    builder.HasOne(h => h.PuestoNavigation).WithMany(p => p.Historiales).HasForeignKey(h => h.PuestoId).OnDelete(DeleteBehavior.SetNull);
    builder.HasOne(h => h.TurnoNavigation).WithMany(t => t.Historiales).HasForeignKey(h => h.TurnoId).OnDelete(DeleteBehavior.SetNull);
    builder.HasOne(h => h.UsuarioNavigation).WithMany(u => u.Historiales).HasForeignKey(h => h.UsuarioId).OnDelete(DeleteBehavior.SetNull);
  }
}
