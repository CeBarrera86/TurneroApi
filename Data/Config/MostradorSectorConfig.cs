...
public class MostradorSectorConfig : IEntityTypeConfiguration<MostradorSector>
{
  public void Configure(EntityTypeBuilder<MostradorSector> builder)
  {
    ...
    builder.HasOne(ms => ms.Mostrador).WithMany(m => m.MostradorSectores).HasForeignKey(ms => ms.MostradorId).OnDelete(DeleteBehavior.Restrict);
    builder.HasOne(ms => ms.Sector).WithMany(s => s.MostradorSectores).HasForeignKey(ms => ms.SectorId).OnDelete(DeleteBehavior.Restrict);
  }
}